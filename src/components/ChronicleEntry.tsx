    ...
    <li className="relative group min-w-0" style={{ "--gold": "oklch(0.72 0.13 80)" } as CSSProperties}>
      ...
      <div
        className={cn(
          ...
          isOpen && "ring-2 ring-gold/50 shadow-[0_0_24px_oklch(0.72_0.13_80/0.4)]",
          isOpen && "border-gold/50 bg-obsidian/90"
        )}
        style={{
          boxShadow: isOpen
            ? "0 0 32px -4px oklch(0.72 0.13 80 / 0.35), 0 0 64px -8px oklch(0.72 0.13 80 / 0.2)"
            : undefined,
        }}
      >
        <button
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3 mb-2">
              <time
                className="font-body text-xs uppercase tracking-[0.2em] text-gold/70 whitespace-nowrap sm:text-sm"
                dateTime={event.date}
              >
                {formatDate(event.date)}
              </time>
              <span
                className="px-1.5 py-0.5 rounded text-[0.55rem] font-display uppercase tracking-[0.15em] text-gold/60 border border-gold/20 bg-obsidian/50 sm:px-2 sm:text-[0.65rem]"
              >
